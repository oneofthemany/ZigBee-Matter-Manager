...
#### 4.1b Queue Navigation and the Player Interface

A zone is addressed as an ordinary player, `zone:<gid>`, and routes through the same controller as a Cast speaker or a WiiM device. This is not cosmetic plumbing: queue, lyrics, favourites, artist actions and the restart-resume path are all written once against the player interface, and a zone that is not a player has to have each of them reimplemented against the session API instead. The adapter is `modules/media/players/zone.py`; the interface it satisfies is `PlayerProvider`.

**The controller must not drive a zone item by item, and the flag that says so is load-bearing.** Every other provider is a transport the controller feeds one track at a time: it plays an item, watches for the device to report the track ended, and plays the next. A zone cannot work that way, because a session start is a full teardown — re-prime, re-launch, re-acquire lock — and doing that per track would put a multi-second silence and a fresh alignment transient at every item boundary, which is precisely what §4.1's seam mechanism exists to avoid. A provider therefore declares `self_advancing`, and the controller then: hands over the whole queue in one call (`play_queue`), suppresses its own auto-advance, and navigates by index (`skip_to`) rather than by re-issuing playback. The queue it keeps is a mirror for the UI, re-pointed from what the engine reports playing on each poll.

**Shuffle has to be materialised as an order, not left as a policy.** The controller's queue implements shuffle by picking a random unplayed index whenever it is asked what comes next — a decision taken per advance. The engine is handed a list and walks it, so a shuffle that lives in the controller would mean the two hold different orders and every index the controller sends to `skip_to` addresses the wrong track. Shuffle is therefore applied by reordering the queue at the moment it is pushed (current item first, remainder randomised), after which both sides hold one order and navigation is linear on both. Repeat-all travels as the engine's existing `loop` flag. Repeat-one has no engine equivalent and is not offered.

**A skip is a seam, and is charged as one.** Moving the queue cursor kills the decoder, which makes the exit non-zero — so the URL provider re-resolves *the position the caller just set* rather than stepping past it, reusing the expired-URL path (§4.1) exactly as it stands. The two things that must not follow are a restart count and a backoff delay: both belong to the fault path, and the backoff would be heard as a gap at a moment the listener is expecting an immediate change. A skip is also explicitly not a play-out, so it can never set `finished` and stop the session — otherwise skipping past the end of a queue would tear the zone down instead of refusing.

**Skips are heard at arm's length, and this is structural rather than a defect to be fixed later.** The seam lands at the write head, and everything between the readers and that head is audio already committed and, for the devices' own buffers, already sent. The outgoing item therefore plays on for something close to a group latency before the new one arrives. The alternative — truncating the ring back to the readers and splicing there — is the operation §A.2 forbids, for the reason given there: a reader seated inside the source's rework margin overtakes the write head, and the silence committed to cover the overtake latches the decoder against it. A responsive skip needs the readers moved first and the ring reworked behind them, which is a correction-ladder operation, not a queue operation.

**Volume stays a property of each speaker.** The timeline carries no gain, so the provider fans a level change out to the members and declares `fans_out_volume` so the controller does not fan it out a second time over the same list. The level a zone reports is the mean of its members', which is the only figure that survives members being trimmed to different levels by hand.

**Pause is stop.** A shared server-built timeline has no pause: there is no per-device transport to hold, and holding the source would strand every reader against a stationary write head. The provider stops the session and keeps the queue and cursor so resume can re-issue them, which restarts the current item from its beginning. This is the one place the player interface promises something the architecture cannot literally deliver, and the resume is close enough to the promise to be worth the adaptation.
...
| No playable position reported for 30 s | Forced receiver re-LOAD (below), then the rung above |
| Two consecutive steps that did not take | Forced receiver re-LOAD (below), then the rung above |
...
**The cooldown after a step is not a constant, and it is not the group's.** A step moves the reader, but the device reveals it only after playing out what it had already buffered — seconds rather than milliseconds. A cooldown shorter than that does not merely delay the recovery: it guarantees at least one poll of pre-step data, and since the decision is a median over recent readings, the stale readings re-authorise the step that produced them. A flat 7 s against a measured 8.4 s target lag stepped one device +4.2 s and then +4.4 s again 27 s later, having already corrected it; the second step is what put the reader on the write head.

Scaling it to `target_lag` fixes the flat case and leaves the general one. What a device holds undrained is *its own* lag, and the two are equal only for a device that is already aligned — which is exactly the device that is never stepped. A device the ladder moves is by definition behind, and behind by `error` means holding `target_lag + error`; waiting only `target_lag` leaves the entire error's worth of pre-step readings inside the window that decides the next move. The loop then re-issues the same step against the same stale evidence for as many polls as the error is deep, and — because each of those steps is *accepted* rather than clamped — never reaches the rung that could escalate. A 25 s interruption on an 8.4 s group therefore repeats a +25 s step indefinitely, which is the "many resyncs, no recovery" signature. The cooldown is `max(floor, observed_lag + poll_interval)` where `observed_lag` is the reading that authorised the move, capped so that one wrong reading cannot blind the ladder for as long as it was wrong. It applies to every path that moves a reader — step, trim, and re-LOAD; trim and re-LOAD pass no reading and so still resolve to the group target, correctly, since both act on a device that is aligned or about to refill to alignment. The re-LOAD is the strictest case, because it also demotes the device to unacquired, where two readings suffice to authorise a step.

**The step rung needs a failure signal of its own.** Every escalation above was reachable only from a step the timeline was too short to hold, which makes the ladder's exit conditional on the *reader* running out of room rather than on the correction failing. A device with room to move whose output does not follow the reader — a receiver wedged behind an interruption, a decoder holding a stale buffer — is corrected by exactly its measured error, waits out a cooldown that now outlasts its buffer, measures the same error again, and steps again, forever. A step therefore records the error it was aimed at, and the next step is charged as futile if the error is still the same sign and at least half the size. Two consecutive futile steps hand the device to the reload rung. The asymmetry is deliberate: an overshoot (opposite sign) is a different fault and resets the count, as does any poll that lands inside the sensor's noise floor.

**Silence is a fault class, not the absence of one.** The ladder is driven entirely by measured error, so a device reporting no position at all is not merely uncorrected — it is unseen. That state is routine rather than exotic: an incoming call or an assistant takes the receiver out of `PLAYING`, a torn-down media session leaves it `IDLE`, and an HTTP fetch the receiver never re-issues leaves it disconnected with its reader frozen. In each case the device drops out of the poll's results, every rung above becomes unreachable, and the speaker stays in the zone playing whatever it has left for the rest of the session. Observed as: a call arriving on a screened device in the zone, answered elsewhere, after which that device never re-aligns. Each poll therefore stamps the devices that answered, and any device seated but quiet for 30 s enters the same ladder at the reload rung — the answer is the same one, since only a fresh LOAD retrieves a receiver that has stopped following the stream. The threshold sits above a re-LOAD's own re-acquisition, which is stamped as a fresh reading so the reload paces the next escalation instead of racing it.

**An interruption is observable directly, and waiting to infer it is what the listener hears.** Silence is the *symptom*; `player_state` is the cause, and it was being discarded — a device not in `PLAYING` produced the same `None` as one unreachable, stale, or not yet started, so the only backstop was the 30 s sweep above. That threshold is an order of magnitude longer than the event which actually produces this fault: a call rings on a screened device in the zone and is answered on a phone a few seconds later. The receiver never goes quiet for 30 s. It goes quiet for six, resumes *into its own buffer*, and is thereafter exactly six seconds behind for the rest of the session.

Two things follow. The offset an interruption produces is, by construction, past the step rung's forward authority — that budget is `max_precomp − precomp_i − trim_i`, a few hundred milliseconds — so no amount of further evidence makes the cheaper rungs applicable. And the evidence itself is expensive: reaching the reload through measured error costs three polls of median filtering, during which the zone plays the same passage twice, audibly, in two rooms. The state is therefore read on every poll and an interruption longer than one poll enters the ladder at the reload rung immediately, retried on its own floor while the device stays down — which is what recovers a receiver held by a ring that outlasts the first attempt. A LOAD is also the only thing that drops the stale buffer, so it is worth issuing while the device is still held: there is nothing to interrupt, and the audio it would have resumed into no longer exists. The rung is deliberately not routed through the escalation counter: an interrupted device is a local fault with a known cause, and taking the whole zone through a re-alignment because one speaker keeps being rung is the wrong answer at any count.

The variant the state sensor cannot see — a receiver that goes on reporting `PLAYING` while its media clock stalls — is caught by the same reasoning applied to the reader ceiling. An offset with no timeline in front of it to absorb it is not made reachable by a third reading, so two same-signed readings past the ceiling escalate without waiting for the median window to fill.

**Absence is not the bottom of the ladder — it is off it.** Every rung assumes a device that is present and wrong: a step moves its reader, a reload drops its buffer, a re-alignment re-derives the whole group's target. None of them describes a device that is not on the network, and the last is actively harmful, because it re-LOADs every other speaker in the zone — seconds of disturbance each — on behalf of one that cannot receive the LOAD. Observed on 2026-09-10: one speaker left WiFi for 2 m 48 s and the ladder walked silence → reload → reload → re-alignment, taking all five speakers down twice for a device that answered none of it, then repeated the whole sequence 22 minutes later.

The distinguishing signal is precise and was already being discarded: the cast provider failing to *resolve* the device. That separates absence from every kind of misbehaviour, so it parks the device instead — out of measurement, out of the target derivation, out of the acquisition lock, out of re-alignment — and the zone plays on undisturbed with the speakers it still has. A parked device is probed for its return on its own backoff rather than through the ladder, and its pre-compensation and trim are deliberately kept: they are properties of the device and the room, they did not change while it was away, and they are what let it rejoin as a single per-device reload against the group's existing target instead of forcing the re-derivation this exists to avoid.

The three faults are now counted apart, because they have three different fixes: *interrupted* is present but not playing, *reloaded* is present and misaligned, *parked* is not there at all. Only the first two are the correction loop's business.

**A device someone else is using is yielded, not recovered.** The interruption rung assumes a receiver that is still the zone's and was held briefly; a re-LOAD is the cure. That assumption is wrong in two cases, and there the cure is the harm. Another Cast app in the foreground, or the default receiver playing media that is not a zone stream, is visible in the receiver status. A WiiM or other LinkPlay box switched to HDMI-ARC, optical or Bluetooth is not: to Cast it is a receiver that stopped playing, and the LOAD switches it back to the network input, taking the TV's sound off the speaker every ten seconds. Its input is therefore read from the box itself (`getPlayerStatus` `mode`: other streaming protocols, and the whole 40–69 input band rather than a list of named inputs, so an input the API document predates is still an input). LinkPlay boxes are found by probing each Cast host once for that API (`modules/media/linkplay.py`). Every LOAD passes one check in `_launch_stream`/`_reload_stream`; a device that fails it is parked as *yielded*, with the same exclusions as absence, probed every few seconds, and let back in only after a settled free run (longer after an input, which flaps as a TV wakes or sleeps). Starting a zone still takes over another Cast app, as casting to one speaker does, but never a box on another input. An unreachable box answers from its last reading for two minutes: failing open on a box last seen on HDMI is the recast this exists to prevent.

**While a zone plays, a LinkPlay member's input is polled** (`_sweep_inputs`, every 2.5 s, one read in flight per device, ~6 ms on the shared connection). Cast can go on reporting PLAYING from a box that has switched to HDMI-ARC, so without it the only detector is the silence sweep, tens of seconds later. Two foreign readings in a row step the speaker aside, through the same policy as every other path, so one glitched reply does not drop it. The API document does not say which mode Cast plays in, so it is *learned*: a non-input mode read twice while the zone is audibly playing on the box (a lag reading in the last 6 s) is recorded as that box's Cast mode in `data/linkplay_state.json` and never read as foreign again. An input is never learned: beside a PLAYING status it means the status is stale, not that Cast is an input.

**A person outranks every sensor: per-speaker policy and locks.** Stored per device in `data/zone_device_policy.json`, keyed by the Cast id (a lock set on a WiiM card's `wiim:<ip>` is matched to the Cast device at that address). The *mode* says what a change of input or app does by itself: `auto` (the default, not stored) steps aside and rejoins once free; `sticky` turns another input into a lock, so a TV going to sleep does not hand the speaker back mid-game; `reclaim` never yields, which is the behaviour before yielding existed. A *lock*, optionally timed, keeps the speaker out of every zone whatever its mode, and applies at once: locking a speaker the zone is playing on yields it and quits the zone's stream there (never someone else's). *Unlocking is a handback*: for ten minutes the next LOAD goes ahead even over another input, once — otherwise a sticky speaker still on HDMI would re-lock on the spot, and "done gaming, give me the music back" would need a trip to the WiiM app. Set from the lock button on each zone member (Media → OpenZone), `POST /api/media/sync/policy`, or the automation step `media_action: zone_lock` (`lock` / `unlock` / `toggle`, optional `lock_minutes`) — which is how a Zigbee button becomes the lock.

**A rung gated on elapsed time must distinguish "not yet" from "never".** The reload and re-align floors compare against a last-fired timestamp, and initialising that to `0.0` is only ever correct by accident: it reads as "long ago" because `time.monotonic()` is normally large. On a host whose monotonic clock starts near zero — a hub inside the first minutes of uptime, which is precisely when a zone is likely to be started — the comparison suppresses the first escalation of the session, the one that matters. Never-fired is represented as absent, and absent always permits.
...
**A correction is not re-decided until it has landed.** A slew is applied to the stream, so it reaches the listener only after that audio has been served and the device's own pipeline has played it out — tens of seconds for a correction of a few tens of milliseconds at the fast rate. Until then the sensor still reports the error the slew was issued for, because the correction is in flight rather than in the sound. Acting on that reading issues the same correction a second time; the two together overshoot, the next poll reverses, and the device hunts. Observed live: one speaker slewing ±30–71 ms every 20–60 seconds for twenty-seven minutes without converging, while its peers in the same zone slewed once every twenty-six. A pending slew above the threshold was already a bar to issuing another, but that bar lifts as the remainder drains below the threshold — and the remainder then drains at the *gentle* rate, so the door stands open for as long as the device stays outside the band. The hold is therefore a cooldown sized to the correction's own flight: the time to serve the portion above the threshold, plus the device's pipeline, plus a poll, bounded by the same ceiling that keeps any one reading from blinding the ladder. This is the rule §7.1 already applies to steps, which fail the same way for the same reason.

**A correction finishes at the rate it started, and the quiet band has to outrun the drift.** The generator used to re-choose a slew's rate from what was *left* of it, so every fast correction dropped to the gentle rate the moment its remainder crossed under the threshold. At 20 ppm the gentle rate was the same order as the clock drift the rate term leaves behind, so inside the band the correction had no authority over its disturbance and the offset settled where the two balanced — just outside the threshold, crossing it on noise. Observed live: 136 fast slews in ninety minutes on one speaker, exactly balanced in sign (mean +0.1 ms), never net-drifting and never converging — a standing offset of 30–40 ms against its peers. A closed-loop simulation with a 40 ppm residual reproduces it (125 slews, ~24 ms held). The rate is therefore latched when a slew is issued, the hold before re-deciding covers the whole drain rather than only the part above the threshold, and the gentle rate is 200 ppm — a third of a cent, inaudible, and enough to drive the band to ~2 ms against any residual the rate clamp allows. The hold stops *deciding*, not *measuring*: the drift fit adds deliberate moves back, so it is fed straight through a correction in flight.
...
**Acquisition at session start is free, because it can be paid inside the prime.** The two costs a session opens with are independent: the delay line has to fill `delay_s + max_precomp` of timeline before any reader can be seated, and each receiver has to be LOADed and observed before its pipeline latency is known. Paying them in sequence — prime with nothing connected, then LOAD and converge — meant the group was either silent for the sum of the two or, when acquisition ran out its budget first, faded in while the ladder was still pulling devices together. The second is what a listener hears as an echo in the opening seconds: not a defect in the correction, but the correction being *performed in front of them*.

The prime is an ffmpeg pipe filling; the probe is a round trip to five receivers. Nothing couples them. Every device is therefore LOADed the moment the source opens, onto a **silent lead** — PCM zeros cut from no timeline at all, paced to real time exactly as content is. The lead is what makes the measurement possible: the receiver's reported media time counts from the first byte of the fetch, so against the moment the generator opened, the difference is the device's whole pipeline latency — network, receiver buffer, decoder, DAC — measured on the device itself, this session, before a sample of content exists. The model's `lag_s` only ever estimated the same quantity from previous sessions, and it estimated it *against the shared epoch*, so it also carried the LOAD skew between devices; the probe does not.

Content starts when the buffer is deep enough **and** the spread between devices is known, and the readers are then seated with that measured spread already pre-compensated. Only the differences are an alignment: the slowest device sets the group's clock and every other reader is seated `slowest − latency_i` further back, which makes the wall-clock time at which device *i* emits timeline sample *s* equal to `epoch + delay_s + s/RATE + trim_i + slowest` — free of `latency_i`, and identical across the group but for the trim, which is the offset the listener asked for. The first content sample is aligned; there is nothing left to discover in public.

Three constraints keep it honest. The device is **not** re-LOADed at the changeover — the lead and the content are one fetch — so the media clock does not restart, and the length of the lead has to be subtracted from the reader's base or every lag reading for the rest of the session is wrong by it. Nothing may be read until the lead has drained: until then the device is reporting from inside the silence, and deriving the group's target from those readings would define "aligned" as the middle of the changeover — the same hold, for the same reason, as a re-alignment's. And the phase is bounded at both ends: a device that never reports is left behind rather than holding the zone silent, and past the cap the learned model is a better answer than more waiting.

A complete settled probe is itself the lock, so the fade-in is released with it. Holding the group silent for two more polls to re-derive from content what the probe has just measured would put the saving straight back.

**A probe cannot tell a slow start from a deep pipeline, so the model must.** The reading is `(now - opened_at) - reported media time`. A device that takes seconds to *begin* playing, then reports normally, produces a constant difference exactly like a device with a genuinely deep output pipeline — it settles, it passes the stability gate, and it is banked. Field readings from one zone make the confusion plain: a speaker measured 454, 500, 586, 747, 806, 1082 and 1328 ms across sessions, and also 7898, 7908 and 7913 ms. A pipeline latency is a property of the hardware and cannot vary seventeen-fold; the large readings are that device's startup on the days it was slow, and which speaker they land on changes session to session.

The separation is one-sidedness. Startup delay can only make a reading *larger* — nothing begins playing faster than its own pipeline — so the pipeline is the **floor** of what a device has been seen to give, not the average of it. An average of a one-sidedly contaminated population sits between the two answers and recognises neither: on the readings above it lands near 2.9 s, which is both wrong as a latency and loose enough to admit the very readings it should reject. The floor is therefore taken over a window of recent sessions, kept as the readings themselves rather than as a running minimum — that is what lets it rise again, since a device that genuinely slowed has its fast sessions age out. A later reading several times that floor is treated as this start rather than as the hardware: the device is seated from the model and the reading is not learned, because a few slow starts fed back into an estimate pull it up to meet them.

A store written before the window existed carries only the average, contaminated upward by the same slow starts. It seeds the window as a single reading, so the floor can begin no higher than it and falls as honest readings arrive. Replayed against the readings one deployment actually produced, every device reaches its true floor within two sessions and admits no slow start after that, while the one genuinely deep pipeline in the zone is never rejected at all.

**No single reading may set the zone's target.** The target is `max(lag) + margin`, every other device is pre-compensated onto it, and nothing downstream questions it — so one device reporting a lag the delay line could never hold takes the whole zone with it for the life of the session. The same deployment produced targets of 106.81 s and 296.77 s against a 20 s ring, which cannot be served at all. The target is therefore bounded by what the delay line can actually seat a reader against, and a device asking for more is left to the correction ladder, which already knows how to reload and ultimately park what it cannot reach. Bounding is not a substitute for measuring correctly — it is what keeps one bad measurement from being unrecoverable.
...
**The identity is asked of the ecosystem that owns the device.** A zone can span ecosystems, and each names its hardware in its own vocabulary and its own field. Cast reports a marketing name over mDNS; LinkPlay reports a build string, `project`, from `getStatusEx`. Neither is reachable from the other's provider, so the key is obtained by resolving the device's `player_id` prefix to its provider and asking that provider. A provider with no stable name for its hardware returns the empty key and opts out entirely — the same "no default beats a wrong default" rule as above. This also decouples the default from the address: a LinkPlay `player_id` is its IP, so without a hardware-level key a DHCP lease change silently discards everything established for that unit.

**A shipped table of priors, beneath everything learned.** The offset cannot be measured from software at all. The pre-roll probe observes the chain down to the position the device reports and pre-compensates it automatically (§7.3); the offset is precisely the remainder — output buffer, DSP, DAC, amplifier, transducer — and no field in either control protocol exposes it. That is what the acoustic sensor is for (§6.2), and why a deployment without one must be told the number once. Since the number is a constant of the hardware, the answer for a model already established elsewhere need not be rediscovered on every install, so known values ship in a table keyed the same way. It is a prior and nothing more: an explicit per-device offset wins, then an offset learned on this network, then the table. It is matched on a normalised form of the key, because one model's Cast name and its LinkPlay build string differ in case, separators and a trailing chipset qualifier that does not change the output path. The table stays reachable after a model's learned default is discarded as positional: that dispute is between units each carrying an explicit offset, which the table never reaches, while an untrimmed unit of the model still has the hardware constant as its best starting point.

#### 7.5 Recovery Re-measures Rather Than Re-modelling

The pre-roll (§7.3) exists because alignment discovered in front of the listener is heard as an echo. Every rung of the ladder that re-LOADs a device was nevertheless re-seating it the way session start worked *before* the pre-roll: from `_model[...]["lag_s"]`, a figure averaged over previous sessions which — having only ever been estimated against the shared epoch — carries the LOAD skew between devices along with it. §3 measures the devices this matters for as varying by most of a second from session to session, so the seat was wrong by an amount the model could not predict, and the device was then released straight onto content for the ladder to pull into place while the zone played. That pull-together is audible in every room the zone covers. The symptom is an echo following any WiFi disturbance; the cause is that recovery had none of the three protections session start has.

A reload therefore opens the same silent lead a session start does, on the fetch the LOAD creates, and reads the device's pipeline latency off it with the same estimator. The costs are asymmetric in the fix's favour: the lead is silence in one room, on a device that was already going to be silent there while its receiver refilled, against an echo everywhere else.

**The measured latency is converted through the target, not through the group's slowest device.** Session start computes `precomp_i = slowest − latency_i` because it is establishing what "aligned" means; it has no target yet and derives one from the lags its seating produces. A reload is the opposite case — the target exists, the rest of the zone is already sitting on it, and the only question is where this one reader goes. A fresh seat satisfies `lag = delay_s + precomp + latency` (the trim appears in the seat and again in the measurement, and cancels — §6.1), so the pre-compensation that lands the device exactly on `target_lag` is `target_lag − delay_s − latency`. Aiming at the operating point directly is also what avoids re-inheriting `STREAM_LAG_MARGIN_S`: seating by the session-start formula mid-session would leave the device a margin's width off target and hand the ladder 350 ms of audible correction to perform.

The quantity is deliberately not clamped at zero. A device slower than the one that set the target needs to be served *newer* audio to come out on time, which is an ordinary negative pre-compensation; the only real bound is the write head, and `_seat_position` already clamps against it and warns (§A.2). A device far enough past that bound to be clamped is one the reload cannot converge, which is the condition §7.1 hands to re-alignment — reached on its own evidence rather than by this seat pretending to succeed.

Three constraints keep it bounded. The probe is skipped outright when there is no target to aim at, which leaves the model path exactly as it was. A probing device is removed from the monitor's measurement rather than merely held in cooldown, because its reported media time counts from the start of the lead and every lag derived from it describes the lead; for the same reason it is skipped by the interruption and silence sweeps, which would otherwise read a device that is silent by design as the fault they exist to catch. And the lead is capped at `STREAM_PROBE_MAX_S`, after which the device is seated from the model as it is today — the cap covers the LOAD as well as the probe, so a launch that never lands cannot hold the lead open.

The reading is not written to the learned model. It is taken from a device disturbed badly enough to need a reload, and the model seeds every future session's pre-compensation; this is the same rule §A.4 applies to re-alignment figures.

**Group re-alignment re-enters the pre-roll rather than imitating it.** The same argument applies with more force one rung up, because a re-align re-LOADs every receiver at once and so performs the whole group's pull-together on content. It cannot use the reload's arithmetic: the target is the thing being re-derived, so there is nothing to aim at. It uses session start's instead — every device onto the silent lead, `precomp_i = slowest − latency_i` from the measured spread, and the target left unset for the monitor to derive from the lags that result, which is the derivation §7.1 credits with converging. This is simply the session-start path re-entered mid-session, and it reuses it directly rather than reimplementing it.

Three things differ from session start, and each is a branch rather than a copy. The delay line is already full and maintained, so the phase ends when the measurement settles instead of when the source finishes priming. The readings must not reach the model, for §A.4's reason. And a reader seats on *leaving* a lead, so a device that never re-fetches would silently keep its pre-realign seat — at session start there is no seat to keep and the generator's own unseated path covers it, but here the fallback has to be explicit.

**The zone is muted for the re-acquisition.** Holding the output at zero until the probe settles is what turns the disturbance from an echo in every room into silence in every room, which is the trade commercial systems make; the fade is released the moment a complete settled probe exists, which is the same lock §7.3 uses, so the silence lasts the probe and not a poll longer. Every device is playing the lead for most of that window anyway — the mute covers only the margin either side, including the interval before a receiver takes its LOAD, where its old stream is still being served.

**The silence is bounded by one budget spanning both phases, and it is not session start's.** A re-align has two ways to hold the zone quiet — the lead, and the acquisition after it — and session start bounds them separately because there they are genuinely separate costs paid before anyone is listening. Mid-track they are one uninterrupted silence: a lead that runs to its cap and then hands a *fresh* acquisition budget to the fade is 35 s of nothing at the default settings, which is far worse than the echo it replaces. `STREAM_REALIGN_MAX_S` therefore replaces `STREAM_ACQUIRE_MAX_S` for the duration and is timed from the re-align itself rather than from the end of the lead, so the two phases divide one bound however they like and the total is what it says. A lead that spends the whole budget leaves the acquisition none, which is correct: the fade-in happens immediately and the ladder converges on audible content, because at that point more silence is no longer buying alignment. The value is sized to still measure the slowest device in the reference deployment (§3: ~6.3 s of pipeline, plus the settling window and a LOAD round trip).

**A parked device must be excluded from the spread.** The pre-compensation is computed against the slowest measured device, so an absent unit carrying a stale probe from session start would define that slowest and seat every other reader back by its whole latency — a seat deep enough to be clamped, and audible on every speaker that is present. Absence is already excluded from measurement, the target derivation, the acquisition lock and the re-LOAD itself (§7.1); the spread is the same rule applied to the one derivation that reads a device's stored measurement rather than a live one.
...
7. **On the default receiver, endpoint displays show the session, not the item.** Track metadata and artwork ride on the load message that starts a device's stream, and closed endpoints offer no way to revise them afterwards; re-issuing the load would restart the device's buffering and cost the zone its alignment (§7.3), and at every track boundary it would exhaust the reload budget §7.1 measures rather than merely being expensive. A zone playing a multi-item queue therefore labels its screens with the **set** — the album, playlist or mix, or failing a name for it the queue's length — never with the head track, which would otherwise sit on every screen for the whole session and read as a stuck player. The controlling application tracks the individual item, where doing so is free.

   In **receiver mode** the limitation does not apply: the custom receiver draws its own screen, so the engine pushes each item over the session WebSocket stamped with the server-clock instant the seam is *heard* — the same clock the receiver already estimates to schedule its audio (§4). The display therefore turns over with the speakers rather than with the decoder, which is a delay line and a device buffer ahead of them, and it costs the audio path nothing. This is the only per-item display path: it needs an App ID registered for the receiver page (`media.cast.sync.app_id`, see `static/cast/README.md`).
...
**A late joiner is seated only where it can be corrected from.** A device that was absent at session start rejoins against the target the group already derived, which is right — re-deriving it would cost the whole zone silence for a device that may be one of six. But the target was set without this one, so it may ask for a seat the timeline cannot give: a joiner *slower* than the target allows needs audio newer than has been written, and a joiner needing more history than the ring holds needs audio already discarded. Clamping it to the nearest edge, which is what an unchecked seat does, leaves it playing early with no move of the reader able to bring it back — the one late-joiner failure the correction ladder cannot undo. The seat is therefore tested against both ends of the delay line before it is taken, and a device that does not fit escalates to a re-align, which re-derives the target with it included.

**Returning is not failing.** The re-align budget counts reloads that did not fix a device the ladder could not reach; past a couple, a reload is known not to work and only re-deriving the target will converge. A device that was simply switched off has not failed at any of that, so the reload that brings it back is not charged against the budget — otherwise a speaker powered on mid-session arrives one ordinary correction away from muting the entire zone.

**Membership is a capability, not an id.** A zone can only hold devices it can both serve and measure; a device it can play to but not read back is an open loop, and the whole system is a closed one. That is a property of the ecosystem, so the engine asks the provider rather than matching on the id's prefix. A prefix test has to be edited every time another ecosystem learns to carry a timeline, and until it is, it silently drops the devices it has not heard of. The same answer decides what a zone will accept as a member and what the interface offers as groupable, so the two cannot drift apart. Native grouping is a separate capability: an ecosystem whose own firmware syncs its members needs no timing help here, and is offered as itself rather than as a zone.

**One box is one box, however many ways it answers.** A single speaker can be discovered by two ecosystems at once — a WiiM Ultra answers Cast discovery and LinkPlay both — and then arrives as two players, with two ids, two model keys, and two sets of learned timing that know nothing about each other. A listener who aligns it through one has not aligned it through the other, and the two can be learned to disagree about the same hardware. The output pipeline a trim describes belongs to the box, not to the protocol it was reached through, so the device's address — the one identity both ecosystems can agree on, neither vendor's id meaning anything to the other — ties them together, and an offset set through either applies to both. A provider that cannot name a device's address says so and opts out: no identity is safer than a shared one, since a wrong match hands one speaker's timing to another.

#### 7.6 Relating Models That Were Never Heard Together

A trim is only ever established by aligning a device against another one in the same zone, so what the listener produces is a relation, not an absolute: this hub needs 81 ms more than that streamer. Recording only the absolute throws that structure away, and every new model then has to be aligned against whatever happened to be playing beside it. Recording the relation instead makes alignments compose — a WiiM aligned against a hub and a Mini aligned against the same hub leaves the WiiM and the Mini related, without those two ever having shared a zone. Each new model costs one alignment and is then on the same scale as the whole fleet.

The store is a graph: models are nodes, observed differences are edges, and absolutes are recovered by weighted least squares against whatever the installation already knows — trims learned here first, the shipped table second, both entering as soft rows rather than constraints so a model with a weak prior and several strong edges is pulled toward its edges. The least squares is there for the cycles. Three models aligned pairwise will not agree to the millisecond, and a graph walked from an anchor would let whichever edge was traversed last absorb the entire closing error; solving distributes it across the observations that disagree, which is the only treatment that does not depend on traversal order.

**An edge measures placement as well as hardware.** A trim set by ear carries the output pipeline *and* where that speaker stands relative to the listener, and a difference of two trims carries the difference of both. Distance is irreducible here — a metre is 3 ms — so an edge earns trust only once repeated observations of the same model pair agree within the same 25 ms the per-model default uses. A pair whose observations scatter is measuring the room, which does not transfer to a device standing somewhere else, and it is dropped rather than averaged: averaging a placement term produces a confident number describing nowhere.

**A component with no absolute has a shape but no position.** Models related only to each other, with nothing in the group carrying a learned or shipped value, are left out of the answer entirely. The graph knows their spacing and genuinely does not know where they sit, and emitting a number for them — zero, or whatever the ridge term settles on — would be indistinguishable to every caller from a measurement.

Only explicit trims are observed. An effective trim that the graph itself supplied would feed its own output back in as evidence, hardening a derivation into a measurement over a few sessions.
...
#### 7.7 Alignment by Ear, as a Bisection

Without a microphone the listener is the only sensor that observes the quantity the offset describes, and the way the control has always been exposed wastes them. A millisecond slider asks for a number, and a number is what a listener cannot supply: nobody hears two speakers and knows that they are 81 ms apart. What a listener *can* do, reliably and at a few milliseconds, is say which of two clicks came first. That is one bit, and one bit per question is enough if the questions are chosen well.

So the control is a bisection. Each round fires an identical click train on both speakers, the subject's delayed by the midpoint of the current bracket, and the listener answers *subject*, *reference* or *together*. Eight rounds take a ±400 ms bracket to about 6 ms, which is where the answers stop being repeatable anyway. The trains are built from one array handed to both devices, so a difference in timbre cannot be mistaken for a difference in arrival, and each probe is a train rather than a single pair because a repeated event is judged and a solitary one is guessed.

The arithmetic is why the answer drops straight into the existing store. The subject is heard later than the reference by `delta + E`, where E is the unknown difference in output latency between them; the search converges on `delta = -E`, and the trim that makes them coincide is the subject's current trim plus exactly that. The result is applied through the ordinary offset path, so it settles, propagates to other units of the model, and is recorded as an edge of the differential graph (§7.6) with no separate bookkeeping.

**An inconclusive search is refused rather than rounded.** Answers that contradict each other leave the bracket open when the round budget runs out. That is evidence about the test, not about the speakers — they may be far enough apart that the room arrives before the direct sound, or close enough that no delta separates them — and the midpoint of a bracket that never narrowed describes nothing. The offset is left where it was.

**"Together" ends the search where it stands.** A listener who cannot separate the two is reporting that the bracket is already inside their resolution. Continuing would record their coin-flips as measurements.
