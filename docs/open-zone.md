...
The separation is one-sidedness. Startup delay can only make a reading *larger* — nothing begins playing faster than its own pipeline — so the pipeline is the **floor** of what a device has been seen to give, not the average of it. An average of a one-sidedly contaminated population sits between the two answers and recognises neither: on the readings above it lands near 2.9 s, which is both wrong as a latency and loose enough to admit the very readings it should reject. The floor is therefore learned separately, relaxing upward slightly each session so a device whose firmware genuinely slowed is not pinned forever to one lucky measurement, and a later reading several times that floor is treated as this start rather than as the hardware: the device is seated from the model and the reading is not learned, because a few slow starts fed back into the estimate pull it up to meet them.
