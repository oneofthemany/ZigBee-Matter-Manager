...
7. podman build --format docker --build-arg BUILD_JOBS=${nproc} \
   ...
   - status updated live from the builder's step counters (20–85%)
   - killed and marked failed past build_max_hours (default 3)
8. podman tag ${image}:${version}-${arch} ${image}:latest-${arch}
9. Tag every named stage as ${image}-stage-<stage>:cache
10. Write status: ready_to_swap / 100% / "Image ready"
11. Clean up clone directory
```
...
- **Cold cache (any host)**: 15–35 minutes (full toolchain rebuild; a 4-job amd64 host measured 33.6 min, most of it the OTBR compile)
...
If the cache becomes corrupted, `podman system prune -a` will force a full rebuild on the next upgrade (worst case: a 35-minute first build).

**Persisting is not the same as surviving GC.** Intermediate stage images are untagged, and `do_gc`'s dangling sweep runs after every swap, so a stage is only kept if something tags it first. Two gaps followed from that, both now closed:

- Stages were tagged only after a *successful* build. A failed or cancelled build left finished stages (the OTBR and wheel compiles) dangling for the next sweep. The sweep now spares dangling images younger than `dangling_min_age_h` (default 48, in `version.json`). Tagging on the failure path was rejected: `--target <stage>` resumes an unfinished stage rather than labelling a finished one, which on cancel restarts the compile the user just stopped.
- The tag list was hardcoded, so any newly added stage was never tagged. It is now read from the Containerfile's `FROM … AS <name>` lines.

**An upgrade is performed by the outgoing version's `upgrade.sh`.** A fix to `upgrade.sh` itself takes effect one upgrade later. The Containerfile is the exception: it is generated by the *target* tag's `build.sh`, so Containerfile changes apply to the very next build.
...
A consequence: the base stays at whatever digest was first pulled, including its bundled pip — which is why pip announced a newer release on every install. That notice is suppressed with `PIP_DISABLE_PIP_VERSION_CHECK=1` (and the root-user warning with `PIP_ROOT_USER_ACTION=ignore`) in every Python stage. Packages installed by `apt-get` inside the stages are current as of their last rebuild; the base layer itself is not refreshed.
...
Everything except the application itself is a final `runtime-base` stage: apt runtime packages, the SiLabs debs and OTBR's files, `cloudflared`, and the Python dependencies. The app image is `FROM runtime-base` and adds only the Rust wheels and `COPY . .`, so a warm upgrade re-runs nothing but that. Layers are ordered rarest-change first. OTBR sits *below* the Python dependencies rather than at the end because its runtime packages need `apt`, and `requirements.lock` changes more often than `OTBR_COMMIT` — with OTBR last, every dependency bump would re-run an apt transaction. The Rust wheels stay in the app image because they are conditional, and nothing conditional may precede the dependency install.

The runtime image carries libraries, not OTBR's build toolchain. When OTBR moved into its own stage its compile dependencies were left in the app image's apt list, so every cold build installed `cmake`, `ninja-build`, the Boost headers and the mbedtls/dbus/avahi/readline/netfilter/systemd `-dev` packages twice. They are replaced by the runtime `.so` packages those headers were transitively providing (`libmbedtls14`, `libdbus-1-3`, `libavahi-client3`, …); the existing `ldd` check fails the build if an OTBR binary is left with an unresolved library. Two things deliberately stay:

- `build-essential`, `libffi-dev`, `libssl-dev`, `pkg-config` — `/api/system/dependencies/install` runs `pip install` inside the live container with arbitrary specs, and some locked packages have no wheel (`netifaces` on any arch, `miniaudio` on arm64).
- `git` — `live_edits` runs `git status` to count local edits.

Stage images end up untagged, which `do_gc`'s dangling sweep would delete. Both `build.sh` and `upgrade.sh` therefore re-run the build for every named stage in the Containerfile after a successful build — a cache hit — and tag it `zigbee-matter-manager-stage-<stage>:cache`. When a pin or crate changes, the superseded stage image loses that tag, goes dangling and is collected normally once older than `dangling_min_age_h`.
