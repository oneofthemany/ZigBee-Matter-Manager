...
def register_system_routes(app: FastAPI, get_zigbee_service, get_mqtt_service, get_manager):
    ...
    # HTTPS is always on (cert auto-generated at boot by ssl_bootstrap) —
    # the /api/ssl/toggle + /api/ssl/status endpoints and their Settings
    # switch were removed deliberately: there is no supported HTTP mode.