...
| `test_wiim_panel.py` | The WiiM device panel against a WiiM Ultra's real replies: inputs decoded from `plm_support` (network, line-in, BT, optical, HDMI, phono), identity, placeholder metadata blanked, a bare-number `getShutdown`, sections dropped rather than failed on older firmware; every action sent as the exact documented command and every invalid one (an input the box lacks, preset 13, sleep 0, an injected `;`) refused before reaching it; a `Failed` reply raised; the automation step and route wiring (read from source) |
